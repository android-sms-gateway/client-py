...
- **Outgoing MMS support** — send MMS messages with optional subject, text, and attachments; MMS fields are encrypted end-to-end when an `Encryptor` is configured
  ```python
  from android_sms_gateway import client, domain

  message = domain.Message(
      phone_numbers=["+12025550100"],
      mms_message=domain.MmsMessage(
          subject="Hello",
          text="World",
          attachments=[
              domain.MmsAttachment(
...
- **Message priority serialization** — the `priority` field is now always included in send payloads (default `0` when unset), matching the API wire format used by the other official clients
